...
Panner2in2out::Panner2in2out (boost::shared_ptr<Pannable> p)
	: Panner (p)
{
	// by default, a single "buffer" is enough, because PanDistributionBuffer itself doesn't have any state
	boost::shared_ptr<PanDistributionBuffer> dist_buf(new PanDistributionBuffer());
	left_dist_buf[0] = dist_buf;
	left_dist_buf[1] = dist_buf;
	right_dist_buf[0] = dist_buf;
	right_dist_buf[1] = dist_buf;
